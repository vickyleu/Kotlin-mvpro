        ...
        .retryWhen {
            it
                .zipWith(
                    ...
                    BiFunction<Throwable, Int, Throwable> { t1, t2 ->
                        currentCount = t2
                        t1
                    })
                .flatMap {
...
fun <T> Observable<T>.defPolicy_Retry(
    lifecycle: LifecycleProvider<*>,
    requestFlag: String = "mvpro"
): Observable<T> {
    ...
    rxPermissions.requestEach(*permissions)
        .subscribe {
            if (it.granted) {
                ...
                val toast = ToastUtils.make()
                toast.setDurationIsLong(false)
                toast.show("${it.name} permission request fail")
            } else {
                val toast = ToastUtils.make()
                toast.setDurationIsLong(false)
                toast.show("${it.name} permission request fail")
